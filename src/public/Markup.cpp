		...
		while ( nFromLenRemaining )
		{
			...
			if ( nResult == (size_t)-1 )
			{
				...
				if ( nErrno == EILSEQ  )
				{
					...
					nToLenBytes += nToCharSize;	// 必须得加，否则真正转换时空间不足 modifed by baifm
				}
				else if ( nErrno == EINVAL )
					break; // incomplete character or shift sequence at end of input
				else if (!pToTempBuffer)
				{
					break;	// 防止异常退不出来 modifed by baifm
				}
...
//baiyadong begin
MCD_STR CMarkup::UTF8ToGB2312( MCD_CSTR pszUTF8, int* pnFailed/*=NULL*/ )
{
	// Converts from UTF-8 to locale GB2312 charset
	MCD_STR strGB2312;
	int nMBLen = (int)MCD_PSZLEN( pszUTF8 );
	if ( pnFailed )
		*pnFailed = 0;
	if ( nMBLen )
	{
		TextEncoding textencoding( MCD_T("UTF-8"), (const void*)pszUTF8, nMBLen );
		textencoding.m_nToCount = nMBLen;
		MCD_CHAR* pGB2312Buffer = MCD_GETBUFFER(strGB2312,textencoding.m_nToCount);
		nMBLen = textencoding.PerformConversion( (void*)pGB2312Buffer,MCD_T("GB2312"));
		MCD_RELEASEBUFFER(strGB2312,pGB2312Buffer,nMBLen);
		if ( pnFailed )
			*pnFailed = textencoding.m_nFailedChars;
	}
	return strGB2312;
...
MCD_STR CMarkup::GB2312ToUTF8( MCD_CSTR pszGB2312 )
{
	// Converts locale GB2312 charset to UTF-8
	MCD_STR strUTF8;
	int nMBLen = (int)MCD_PSZLEN( pszGB2312 );
	if ( nMBLen )
	{
		TextEncoding textencoding( MCD_T("GB2312"), (const void*)pszGB2312, nMBLen );
		textencoding.m_nToCount = nMBLen * 4;
		MCD_CHAR* pUTF8Buffer = MCD_GETBUFFER(strUTF8,textencoding.m_nToCount);
		nMBLen = textencoding.PerformConversion( (void*)pUTF8Buffer, MCD_T("UTF-8") );
		MCD_RELEASEBUFFER(strUTF8,pUTF8Buffer,nMBLen);
